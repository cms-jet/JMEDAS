...
JetMiniValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (const pat::Jet &ijet : *AK4CHS) {  
    ...
    // double bdisc_CSV        = ijet.bDiscriminator("pfCombinedInclusiveSecondaryVertexV2BJetTags");
    // double bdisc_MVA        = ijet.bDiscriminator("pfCombinedMVAV2BJetTags");
  ...
  for (const pat::Jet &ijet : *AK8MINI) {  
    ...
    cout<<"sdSubjetsPuppi size "<<sdSubjetsPuppi.size()<<endl;
    int count_pup=0;
